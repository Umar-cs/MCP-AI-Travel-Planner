...
OPENROUTER_FALLBACK_MODEL_ID = "openrouter/auto"
...
BUNDLED_PNPM_PATH = r"C:\Users\itxum\.cache\codex-runtimes\codex-primary-runtime\dependencies\bin\pnpm.cmd"
...
def build_search_tool():
    """Return the first available Agno web-search tool."""
    if WebSearchTools is not None:
        return WebSearchTools()
    return None


def build_mcp_runner_command(package_name: str, *args: str) -> str:
    """Build a package-runner command for MCP servers on machines without npx."""
    if shutil.which("npx"):
        parts = ["npx", "-y", package_name, *args]
    else:
        pnpm_path = shutil.which("pnpm") or (BUNDLED_PNPM_PATH if os.path.exists(BUNDLED_PNPM_PATH) else None)
        if not pnpm_path:
            raise RuntimeError("Node package runner not found. Install Node.js/npm or set PNPM on PATH to run MCP servers.")
        parts = [pnpm_path, "dlx", package_name, *args]

    return " ".join(f'"{part}"' if " " in part else part for part in parts)


def safe_error_message(error: Exception, *secrets: str) -> str:
    message = str(error)
...
async def run_mcp_travel_planner(destination: str, num_days: int, preferences: str, budget: int, openrouter_key: str, google_maps_key: str, model_id: str = DEFAULT_OPENROUTER_MODEL_ID):
    ...
    try:
        ...
        mcp_tools = MultiMCPTools(
            [
                build_mcp_runner_command("@openbnb/mcp-server-airbnb", "--ignore-robots-txt"),
                build_mcp_runner_command("@gongrzhe/server-travelplanner-mcp"),
            ],      
            env={
        ...
        travel_planner = Agent(
            ...
            model=OpenAIChat(
                ...
                default_headers={
                    "X-OpenRouter-Title": "MCP AI Travel Planner",
                },
            ),
            description=dedent(
                """\
        ...
        **REQUIRED OUTPUT FORMAT:**
        ...
        try:
            ...
            if model_id == OPENROUTER_FALLBACK_MODEL_ID:
                raise RuntimeError(safe_error_message(error, openrouter_key, google_maps_key))

            fallback_planner = Agent(
                name="Travel Planner",
                role="Creates travel itineraries using Airbnb, Google Maps, and Google Search",
                model=OpenAIChat(
                    id=OPENROUTER_FALLBACK_MODEL_ID,
                    api_key=openrouter_key,
                    base_url=OPENROUTER_BASE_URL,
                    provider="OpenRouter",
                    supports_native_structured_outputs=False,
                ...
                description=travel_planner.description,
                instructions=travel_planner.instructions,
                tools=agent_tools,
                add_datetime_to_context=True,
                markdown=True,
                debug_mode=False,
            )
            fallback_response: RunOutput = await fallback_planner.arun(prompt)
            fallback_content = (fallback_response.content or "").strip()
            if not fallback_content:
                raise RuntimeError(
                    "OpenRouter model attempts failed. "
                    f"{model_id}: {safe_error_message(error, openrouter_key, google_maps_key)} | "
                    f"{OPENROUTER_FALLBACK_MODEL_ID}: empty response"
                )
            return f"> Generated with fallback model `{OPENROUTER_FALLBACK_MODEL_ID}` because `{model_id}` did not complete successfully.\n\n{fallback_content}"

    finally:
        if mcp_tools is not None:
            await mcp_tools.close()

def run_travel_planner(destination: str, num_days: int, preferences: str, budget: int, openrouter_key: str, google_maps_key: str, model_id: str = DEFAULT_OPENROUTER_MODEL_ID):
    """Synchronous wrapper for the async MCP travel planner."""
    return asyncio.run(run_mcp_travel_planner(destination, num_days, preferences, budget, openrouter_key, google_maps_key, model_id))
...
with st.sidebar:
    ...
    openrouter_api_key = st.text_input("OpenRouter API Key", type="password", help="Required for AI planning through OpenRouter")
    google_maps_key = st.text_input("Google Maps API Key", type="password", help="Required for location services")
    selected_model_id = st.selectbox(
        "OpenRouter Model",
        OPENROUTER_MODEL_OPTIONS,
        index=OPENROUTER_MODEL_OPTIONS.index(DEFAULT_OPENROUTER_MODEL_ID),
        help="Gemini 2.5 Flash is the default because Owl Alpha returned blank/provider responses during testing.",
    )

    # Check if API keys are provided (both OpenRouter and Google Maps are required)
    api_keys_provided = openrouter_api_key and google_maps_key

    if api_keys_provided:
        st.success(f"✅ All API keys configured! Using {selected_model_id}.")
    else:
